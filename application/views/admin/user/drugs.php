...
<script>
    ...
    {
        var elmId = $("#check_box_"+data).val();
        
        if(elmId == 'no'){
            $("#check_box_"+data).val('yes');
        }else{
            $("#check_box_"+data).val('no');
        }
  ...
  <section class="content container">
      ...
      <div class="box-body">
        <form id="cat-form" method="post" enctype="multipart/form-data" class="validate-form" action="<?php echo base_url('admin/drugs/add')?>" role="form" novalidate>
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" required name="name" value="<?php echo $drug->name; ?>" >
          </div>

          <div class="form-group">
            <label><?php echo trans('details') ?></label>
            <textarea class="form-control" name="details" rows="6"><?php echo ($drug->details); ?></textarea>
          </div>

          <input type="hidden" name="id" value="<?php echo html_escape($drug->id); ?>">
          <!-- csrf token -->
    ...
    <?php if (isset($page_title) && $page_title != "Edit"): ?>
      ...
      <div id="import_exp" class="pull-right btn btn-secondary btn-sm csv_btn" style="display:none;">
          <form method="post" action="<?php echo base_url('admin/import_file/add_drugs')?>" role="form" novalidate enctype="multipart/form-data">
          <input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>">
    ...
    <div class="box list_area">
      ...
      <div class="box-body">
        ...
        <div class="col-md-12 col-sm-12 col-xs-12 scroll">
            <form method="post" action="<?php echo base_url('admin/drugs/remove')?>">
                
                <input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>">
                <input type="submit" class="btn btn-dark" name="submit_file" value="Delete"/>
                ...
                <tbody>
                  <?php $i=1; foreach ($drugs as $drug): ?>
                    <tr id="row_<?php echo html_escape($drug->id); ?>">
                        ...
                        <td><input type="checkbox" onclick="check_value('<?php echo $drug->id; ?>');" id="check_box_<?php echo $drug->id; ?>" name="check_box_<?php echo $drug->id; ?>" value="no"></td>
                    </tr>
...
document.querySelector("#csvdata").addEventListener("click", function () {
	...
	export_table_to_csv(html, "drugs.csv");
});
